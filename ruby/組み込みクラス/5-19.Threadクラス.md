...
* `kill`：スレッドを終了する。指定したスレッドの`exit`メソッドを呼び出す
...
* `raise`：スレッドで強制的に例外を発生させる。実行したスレッドの中で、指定した例外を発生させる
...
* `list`：実行中のスレッドのリストを取得する。スレッドを生成していない場合は、メインスレッドのみ表示される
...
* `priority`：スレッドの優先度を得る。値が大きいほど、優先度が高くなる

* `priority=`：優先度を設定することができる。メインスレッドのデフォルト値は0で、新しく生成されたスレッドは親スレッドの値を引き継ぐ
